...
def run_recipe_simple_with_config(
    config: EffectiveConfig | dict[str, Any],
    recipe_path: str,
    **kwargs: Any
) -> RunSummary:
    """
    Legacy adapter for run_recipe_simple that accepts config objects directly.
    
    This is the preferred method for programmatic usage as it avoids file I/O.
    
    Args:
        config: Effective configuration object or dict
        recipe_path: Path to recipe YAML file
        **kwargs: Fluent API overrides
    ...
    # Create FMF instance with config object
    ...
    try:
        ...
        notes = f"Recipe failed: {exc}. Check configuration and secrets."
        return RunSummary(ok=False, notes=notes)
